...
describe Cequel::Migrations::Rails::KeyspaceManager do
  describe "#new" do
    ...
    it "creates a CQL connection for an array of hosts" do
      Cequel::Migrations::Rails::KeyspaceManager.double(:cequel_env_conf).and_return({ 'hosts' => ['some host', 'another host'] })
    ...
    it "creates a CQL connection for an array of hosts when the port is specified" do
      Cequel::Migrations::Rails::KeyspaceManager.double(:cequel_env_conf).and_return({ 'hosts' => ['host1', 'host2'], 'port' => '9042'})
      Cql::Client.should_receive(:connect).with(['host1', 'host2'])
      Cequel::Migrations::Rails::KeyspaceManager.new
    end
  end
