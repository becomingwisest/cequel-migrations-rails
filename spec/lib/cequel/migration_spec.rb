...
describe Cequel::Migration do
  ...
  describe "#new" do
    ...
    it "create a cql-rb database connection for the host & keyspace specified in the environment's config if the port is 9042" do
      migration_class.double(:cequel_env_conf).and_return({ 'host' => 'somehost', 'keyspace' => 'somekeyspace', 'port' => '9042' })
  ...
  describe "#execute" do
    ...
    it "delegates to the cql-rb connection execute" do
      migration_class.double(:cequel_env_conf).and_return({ 'keyspace' => 'test keyspace', 'host' => '123.123.123.123','port' => '9042' })
      db = double('db').as_null_object
      Cql::Client.double(:connect).and_return(db)
      db.should_receive(:execute).with("some cql string")
      migration.execute("some cql string")
    end
  end
